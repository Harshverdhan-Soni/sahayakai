...
//  Routes ALL chat through FastAPI → Agent Orchestrator → iHRMS DB → Ollama
//  Backend: http://localhost:8000  (uvicorn main:app --reload)
// ══════════════════════════════════════════════════════════════════

const BACKEND_URL = "http://localhost:8000";

// Call FastAPI backend — it handles DB fetch + Ollama in one call
const callBackend = async (userPrompt, history) => {
  ...
  // Returns: { response, agent_chain, tool_calls, db_context, needs_hitl }
};

// Kept for UI — maps agent_chain from backend to display labels
const classifyIntent = (prompt) => {
  const p = prompt.toLowerCase();
  if (p.includes("leave") || p.includes("cl") || p.includes("el") || p.includes("balance"))
    return { agentChain:["coordinator","leave"],   tools:["fetch_leave_entries","fetch_leave_balances"] };
  if (p.includes("salary") || p.includes("payroll") || p.includes("pay slip") || p.includes("net pay"))
    return { agentChain:["coordinator","hrdata"],  tools:["fetch_salary_records","fetch_deductions"] };
  if (p.includes("grievance") || p.includes("complaint") || p.includes("cpgrams"))
    return { agentChain:["coordinator","rti"],     tools:["fetch_grievances"] };
  if (p.includes("letter") || p.includes("circular") || p.includes("dak"))
    return { agentChain:["coordinator","document"],tools:["fetch_letters"] };
  if (p.includes("attendance") || p.includes("present") || p.includes("absent"))
    return { agentChain:["coordinator","hrdata"],  tools:["fetch_attendance","fetch_holidays"] };
  if (p.includes("employee") || p.includes("staff") || p.includes("promotion") || p.includes("transfer"))
    return { agentChain:["coordinator","hrdata"],  tools:["fetch_employees","fetch_promotions"] };
  if (p.includes("rti") || p.includes("draft") || p.includes("reply"))
    return { agentChain:["coordinator","rti"],     tools:["fetch_rti_application","generate_draft_reply"] };
  if (p.includes("tour") || p.includes("ta/da") || p.includes("travel"))
    return { agentChain:["coordinator","tour"],    tools:["fetch_tour_request","calculate_tada"] };
  return { agentChain:["coordinator"],             tools:["understand_intent"] };
};

// Detect if response needs HITL approval card
const detectHITL = (text) => {
  const t = text.toLowerCase();
  if (t.includes("draft reply") || t.includes("draft note") || t.includes("draft letter"))
    return { type:"approve_draft",  label:"Approve this draft",       rejectLabel:"Reject & redraft" };
  if (t.includes("approve") && (t.includes("leave") || t.includes("application")))
    return { type:"bulk_action",    label:"Approve application(s)",   rejectLabel:"Hold for review" };
  if (t.includes("escalat") || t.includes("reminder"))
    return { type:"approve_draft",  label:"Send escalation/reminder", rejectLabel:"Dismiss" };
  if (t.includes("circulate") || t.includes("send to"))
    return { type:"approve_draft",  label:"Approve & circulate",      rejectLabel:"Save draft only" };
  return { type:"acknowledge", label:"Understood — noted", rejectLabel:null };
};
  ...
  return (
    <button
      ...
      style={{
        ...
        background: copied ? `${C.greenL}22` : `${C.surface}cc`,
        border:`1px solid ${copied ? C.greenL+"66" : C.dim+"55"}`,
        borderRadius:4, padding:"3px 8px",
        cursor:"pointer", fontSize:10,
        color: copied ? C.greenL : C.mid,
        fontFamily:"'DM Mono',monospace", letterSpacing:0.3,
        transition:"all 0.2s",
...
const ChatPanel = ({ onClose, seedPrompt, onSeedConsumed }) => {
  ...
  const [ollamaStatus, setOllamaStatus] = useState("idle"); // idle | connecting | streaming | error
  ...
  const sendMessage = async (text, fromNotif = false) => {
    ...
    // Optimistic UI — show agent chain while backend fetches DB + calls Ollama
    const { agentChain, tools } = classifyIntent(userText);
    const aid = msgIdRef.current++;

    setMessages(prev => [...prev, {
      id: aid, role:"assistant",
      agentChain,
      toolCalls: tools.map(t => ({ tool: t, args: { query: userText.slice(0,50) }, result: "Fetching from iHRMS…" })),
      text: "", hitl: null,
      streaming: true,
    }]);
    ...
    try {
      ...
      // Backend: classifies intent → fetches iHRMS DB → sends to Ollama → returns response
      const data = await callBackend(userText, messages);

      // Update tool calls with real results from backend
      const realToolCalls = data.tool_calls || [];
      const dbBadge = data.db_context
        ? `\n\n---\n📊 *Data source: iHRMS live DB (${realToolCalls.length} tool call${realToolCalls.length!==1?"s":""})*`
        : "";
      const fullText = data.response + dbBadge;

      const hitl = detectHITL(fullText);
      setMessages(prev => prev.map(m =>
        m.id === aid ? {
          ...m,
          text:       fullText,
          agentChain: data.agent_chain || agentChain,
          toolCalls:  realToolCalls.length ? realToolCalls : m.toolCalls,
          hitl,
          streaming:  false,
        } : m
      ));
      ...
      setMessages(prev => prev.map(m =>
        m.id === aid ? {
          ...m,
          text: `⚠️ **Could not reach SAHAYAK-AI Backend**\n\n\`${err.message}\`\n\n**Fix — run these in your backend folder:**\n\`\`\`bash\npip install -r requirements.txt\nuvicorn main:app --reload --port 8000\n\`\`\`\nAlso ensure Ollama is running: \`ollama serve\``,
          streaming: false,
          hitl: null,
        } : m
      ));
      ...
      <div style={{ padding:"12px 16px", background:C.surface,
        ...
        <div>
          ...
          <div style={{ fontFamily:"'DM Mono',monospace", fontSize:9, letterSpacing:1,
            color: ollamaStatus==="error" ? C.red : ollamaStatus==="streaming" ? C.saffron : C.greenL }}>
            <StatusDot ok={ollamaStatus !== "error"} />
            { ollamaStatus==="connecting" ? "⟳ CONNECTING TO BACKEND..." :
              ollamaStatus==="streaming"  ? "⬡ FETCHING iHRMS + LLM RESPONDING..." :
              ollamaStatus==="error"      ? "✗ BACKEND OFFLINE — run: uvicorn main:app --reload" :
              "llama3.2 · iHRMS LIVE DB · BACKEND CONNECTED" }
          </div>
        </div>
        <button onClick={onClose}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                ...
                <div style={{ position:"relative" }}>
                  <div style={{ background:`${C.surface}ee`, border:`1px solid ${C.dim}44`,
                    ...
                    outline: msg.streaming ? `1px solid ${C.saffron}44` : "none", padding:"11px 14px 11px 14px" }}>
                    {msg.streaming && msg.text === "" && <TypingIndicator />}
                    <RenderText text={msg.text} />
                    {msg.streaming && msg.text !== "" && (
                      <span style={{
                        display:"inline-block", width:7, height:13,
                        background:C.saffron, marginLeft:3, borderRadius:1,
                        animation:"pulse 0.7s ease-in-out infinite",
                        verticalAlign:"middle", opacity:0.8,
                      }} />
                    )}
                  </div>
                  {/* Copy button — only when response is complete */}
                  {!msg.streaming && msg.text && (
...
export default function App() {
  ...
  return (
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh",
      ...
      {/* ── TOAST STACK (bottom-right) — max 1 visible, Clear All when >1 queued ── */}
      <div style={{ position:"fixed", bottom:24, right:24, zIndex:2000,
        display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end",
        pointerEvents:"none" }}>

        {/* Clear All button — only shows when 2+ toasts are queued */}
        {toasts.length > 1 && (
          <div style={{ pointerEvents:"all" }}>
            <button
              ...
              style={{
                ...
                background:"#0A2240ee", border:"1px solid #E5393566",
                borderRadius:6, padding:"5px 14px",
                color:"#E53935", fontSize:11, cursor:"pointer",
                fontFamily:"'DM Mono',monospace", fontWeight:700,
                letterSpacing:0.5, backdropFilter:"blur(8px)",
                boxShadow:"0 4px 16px rgba(0,0,0,0.4)",
              }}>
              ✕ Clear all {toasts.length} notifications
            </button>
          </div>
        )}

        {/* Show only the LATEST toast — rest are queued in state */}
        {toasts.length > 0 && (
          <div style={{ pointerEvents:"all" }}>
            <NotifToast
              notif={toasts[toasts.length - 1]}
              onDismiss={() => dismissToast(toasts[toasts.length - 1].toastKey)}
              onAction={handleNotifAction}
            />
          </div>
        )}

        {/* Badge showing how many more are queued behind */}
        {toasts.length > 1 && (
          <div style={{
            pointerEvents:"none",
            fontFamily:"'DM Mono',monospace", fontSize:9,
            color:"#7A90A8", letterSpacing:0.5, textAlign:"right",
          }}>
            +{toasts.length - 1} more notification{toasts.length - 1 > 1 ? "s" : ""} queued
          </div>
        )}
      </div>
...
// // Model: llama3.2 — install with: ollama pull llama3.2

// const OLLAMA_URL   = "http://localhost:11434/api/chat";
// const OLLAMA_MODEL = "llama3.2";

// // Classify prompt intent to build agent chain + tool calls UI
// const classifyIntent = (prompt) => {
//   const p = prompt.toLowerCase();
//   if (p.includes("pending file") || p.includes("summarise") || p.includes("urgent") || p.includes("queue"))
//     return { agentChain:["coordinator","document"],  tools:["list_pending_files","classify_priority"] };
//   if (p.includes("rti") || p.includes("draft") || p.includes("reply") || p.includes("applicant"))
//     return { agentChain:["coordinator","rti"],        tools:["fetch_rti_application","generate_draft_reply"] };
//   if (p.includes("leave") || p.includes("compliance") || p.includes("balance") || p.includes("cl") || p.includes("el"))
//     return { agentChain:["coordinator","leave"],      tools:["fetch_leave_applications","check_leave_balance"] };
//   if (p.includes("mis") || p.includes("report") || p.includes("pendency") || p.includes("weekly"))
//     return { agentChain:["coordinator","mis"],        tools:["aggregate_pendency","generate_mis_report"] };
//   if (p.includes("tour") || p.includes("ta/da") || p.includes("travel") || p.includes("bengaluru"))
//     return { agentChain:["coordinator","tour"],       tools:["fetch_tour_request","calculate_tada"] };
//   if (p.includes("grievance") || p.includes("cpgrams") || p.includes("sla") || p.includes("disposal"))
//     return { agentChain:["coordinator","rti"],        tools:["fetch_grievances","check_sla_status"] };
//   if (p.includes("employee") || p.includes("ihrms") || p.includes("payroll") || p.includes("validate"))
//     return { agentChain:["coordinator","hrdata"],     tools:["fetch_employee_master","validate_service_books"] };
//   if (p.includes("meeting") || p.includes("action item") || p.includes("mom") || p.includes("overdue"))
//     return { agentChain:["coordinator","mis"],        tools:["fetch_action_items","suggest_escalation"] };
//   return { agentChain:["coordinator"],                tools:["understand_intent"] };
// };

// // Build tool calls display objects for the UI
// const buildToolCalls = (tools, prompt) => tools.map(tool => ({
//   tool,
//   args:   { query: prompt.slice(0, 50) },
//   result: "Fetched from iHRMS · MCP tool call complete",
// }));

// // System prompt — governs ALL LLM responses
// const SAHAYAK_SYSTEM_PROMPT = `You are SAHAYAK-AI, an agentic decision-support assistant for Indian government Section Officers.

// CRITICAL RULES — follow these without exception:
// 1. Every response must end with: "[OFFICER APPROVAL REQUIRED BEFORE ANY ACTION]"
// 2. Never claim to have executed any action — you only RECOMMEND.
// 3. Always cite the relevant Act, Rule, or SOP when giving legal/procedural advice.
// 4. For RTI matters, always reference the RTI Act, 2005 and the 30-day statutory deadline.
// 5. For leave matters, reference CCS (Leave) Rules, 1972.
// 6. For tour/TA-DA, reference SR (Supplementary Rules).
// 7. Use formal government language — address the officer as "Sir/Ma'am".
// 8. Format responses clearly using markdown — use **bold** for critical items.
// 9. Always flag risks, SLA breaches, and policy violations explicitly.
// 10. Keep responses concise but complete — under 400 words unless a full draft is needed.

// Context: You have access to iHRMS data (leave, tour, payroll, employee records), RTI tracking system, grievance portal (CPGRAMS), and file management system via MCP tools.`;

// // Call Ollama API with streaming
// const callOllama = async (userPrompt, history, onChunk) => {
//   // Build message history for context
//   const messages = [
//     { role: "system", content: SAHAYAK_SYSTEM_PROMPT },
//     ...history.slice(-6).map(m => ({          // last 3 exchanges for context
...
//       stream:   true,
//       options: {
//         temperature:    0.2,   // low = more consistent, formal government tone
//         num_predict:    600,   // max tokens per response
//         top_p:          0.9,
//         repeat_penalty: 1.1,
//       },
//     }),
...
//         if (chunk) {
//           full += chunk;
//           onChunk(full);   // update UI token by token
//         }
...
//     // Classify intent for agent chain + tool call UI
//     const { agentChain, tools } = classifyIntent(userText);
//     const toolCalls = buildToolCalls(tools, userText);
//     const aid = msgIdRef.current++;

//     // Add assistant placeholder — we'll stream into it
//     setMessages(prev => [...prev, {
//       id: aid, role:"assistant",
//       agentChain, toolCalls,
//       text: "", hitl: null,
//       streaming: true,
//     }]);

//     // Small delay so user sees agent chain animate before text starts
...
//         messages,
//         (partial) => {
//           // Update message text token by token for streaming effect
//           setMessages(prev => prev.map(m =>
//             m.id === aid ? { ...m, text: partial } : m
//           ));
//         }
//       );

//       // Final update with HITL card + mark streaming done
//       const hitl = detectHITL(fullText);
//       setMessages(prev => prev.map(m =>
//         m.id === aid ? { ...m, text: fullText, hitl, streaming: false } : m
//       ));
//       setOllamaStatus("idle");
...
//         m.id === aid ? {
//           ...m,
//           text: `⚠️ **Could not reach Ollama**\n\n\`${err.message}\`\n\n**Fix:** Open a terminal and run:\n\`\`\`\nollama serve\n\`\`\`\nThen try again. Make sure \`llama3.2\` is installed (\`ollama pull llama3.2\`).`,
//           streaming: false,
//           hitl: null,
//         } : m
//       ));
...
//             color: ollamaStatus==="error" ? C.red : ollamaStatus==="streaming" ? C.saffron : C.greenL }}>
//             <StatusDot ok={ollamaStatus !== "error"} />
//             { ollamaStatus==="connecting" ? "⟳ CONNECTING TO OLLAMA..." :
//               ollamaStatus==="streaming"  ? "⬡ LLAMA 3.2 RESPONDING..." :
//               ollamaStatus==="error"      ? "✗ OLLAMA OFFLINE — run: ollama serve" :
//               "llama3.2 · OLLAMA LOCAL · 34 TOOLS" }
//           </div>
//         </div>
//         <button onClick={onClose}
//           style={{ marginLeft:"auto", background:"none", border:`1px solid ${C.dim}66`,
